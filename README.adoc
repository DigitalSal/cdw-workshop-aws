...
1. https://attend.cloudera.com/clouderadataservicesworkshops[AMERICAS Partner Workshops] - A schedule of upcoming Cloudera Hands-On Workshops. +

2. https://www.youtube.com/watch?v=2pvxWb2vvGo[CDW Workshop (Recording)] - Watch this video for the complete walkthrough of this CDW workshop. +
...
Please use the login URL: http://3.23.227.113/auth/realms/master/protocol/saml/clients/cdp-sso[Workshop login]. +
Enter the `Workshop Login Username` and `Workshop Login Password` shared by your instructor. +

(*Note*: Note that your username would be something like `imday_amer###`, e.g. `imday_amer55`). +
...
=== 3.1 Open Hue for your assigned CDW Virtual Warehouse (Hive) - `amer-im-day-01 thru amer-im-day-15` +

- Click on the image:images/step3/hue.png[] button on the right upper corner of your assigned Virtual Warehouse as shown in the screenshot below. +
...
=== 3.2 Run the following DDL in HUE for your assigned CDW Virtual Warehouse +
...
STORED AS TEXTFILE LOCATION 's3a://amer-im-day-buk-9051cc7f/data/airlines-raw/airlines-csv/flights' tblproperties("skip.header.line.count"="1");
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' 
STORED AS TEXTFILE LOCATION 's3a://amer-im-day-buk-9051cc7f/data/airlines-raw/airlines-csv/planes' tblproperties("skip.header.line.count"="1");
...
STORED AS TEXTFILE LOCATION 's3a://amer-im-day-buk-9051cc7f/data/airlines-raw/airlines-csv/airlines' tblproperties("skip.header.line.count"="1");
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' 
STORED AS TEXTFILE LOCATION 's3a://amer-im-day-buk-9051cc7f/data/airlines-raw/airlines-csv/airports' tblproperties("skip.header.line.count"="1");
----
...
=== 3.3 Run the following DDL in HUE for assigned CDW Virtual Warehouse(Impala) - `amer-im-day-01-impala - amer-im-day-15-impala`. +

- Go to the page where now you will access HUE of an Impala virtual warehouse. Click on `HUE` as shown in the screenshot below.
...
- Go back to the CDP Console and observe your assigned Impala Virtual Warehouse. +
...
- Make sure that you are using the HUE of your assign Hive Virtual Warhouse. Else, click on `HUE` and go to the HUE browser. +
...
(a) Location: `s3a://amer-im-day-buk-9051cc7f/my-data/warehouse/tablespace/external/hive/wuser00_airlines.db/planes` +
...
From the cloudera console click on your assigned Impala Virtual Warehouse. +
...
- Open HUE for your assigned Hive Virtual Warehouse
...
- Open HUE for your assigned Impala Virtual Warehouse. +
...
- This can be used for both Iceberg tables and Hive Tables to improve performance. Go to the Cloudera console and look for your assigned Hive Virtual Warehouse. Click on the `Hue` button on the right upper corner as shown in the screenshot below. +
...
- Change the radio button to select appropriate data lake. In this case it is `amer-workshop-env`. +
...
- Test the policy is working - Open `HUE` for your assigned Impala Virtual Warehouse and execute the following query. +
...
- Then click on `Data Visualization` option in the left windowpane. You'll see an option `Data VIZ` next to the data-viz application with the name `amer-workshop-dataviz`. It should open a new window.+
